...
const remoteLocations = ["Port Blair", "Leh", "Aizawl", "Itanagar", "Agartala"]; // Cities in remote states
...
class LogisticsAdvisorService {
    ...
    static getRecommendations(order) {
        ...
        for (const partnerName in partnerRateCards) {
            ...
            // RULE: If destination is remote, only recommend 'air' mode partners
            if (isRemote && partner.mode !== 'air') {
                continue; // Skip land-based partners for Andaman
            }

            // RULE: If not remote but long distance, prioritize standard/premium
            if (!isRemote && isLongDistance && partner.type === 'budget') {
                 // Optionally skip or just let the cost decide
            }
            ...
            recommendations.push({
                partnerName,
                logoUrl: `/${partnerName.toLowerCase().replace(' ', '')}.png`, // e.g., /delhivery.png
                estimatedPrice: Math.ceil(estimatedPrice), // Round up to nearest Rupee
