// backend/populate_coordinates.js
...
// In a production app, you would use a Geocoding API for this.
const cityCoordinates = {
    ...
    "Bhagalpur": { lat: 25.2497, lon: 86.9850 },
...
async function populateUserCoordinates() {
    try {
        ...
        for (const user of users) {
            ...
            // Check if the user already has coordinates to avoid re-processing
            if (city && cityCoordinates[city] && !user.profile?.location?.latitude) {
                const baseCoords = cityCoordinates[city];
                
                // Add a small random offset to simulate different locations within a city
                // 0.1 degrees is roughly 11km, so this scatters users within a reasonable radius
        ...
        if (updatePromises.length > 0) {
            ...
            console.log(`Successfully updated ${updatedCount} users with mock coordinates.`);
        } else {
            console.log("No users needed coordinate updates.");
        }

    } catch (error) {
        console.error("An error occurred while populating user coordinates:", error);
    }
}
