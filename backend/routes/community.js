...
router.get("/", auth, async (req, res) => {
  try {
    ...
    if (!userLocation?.city || !userLocation?.latitude) {
      return res
        .status(400)
        .json({ message: "Your profile location is incomplete." });
    }
    ...
    if (stateAmbassadors.length > 0) {
      const ambassadorsWithDistance = stateAmbassadors.map((ambassador) => {
    ...
    const localArtisans = stateArtisans
      ...
      .map((artisan) => {
        ...
        return {
          id: artisan.id,
          name: artisan.name,
          avatar: artisan.profile?.avatar,
          craft: artisan.artisanProfile?.craftSpecialty?.join(", "),
    ...
    const communityData = {
      ...
      areaAmbassador: closestAmbassador
        ? {
            ...
            avatar:
              closestAmbassador.profile?.avatar ||
              "https://images.unsplash.com/photo-1521146764736-56c929d59c83?auto=format&fit=crop&w=200&q=80",
            title: `KalaGhar Ambassador - ${
              closestAmbassador.profile?.location?.city || "Local Area"
            }`,
            bio:
              closestAmbassador.ambassadorProfile?.bio ||
              "Passionate about empowering local artisans.",
            specialties: closestAmbassador.ambassadorProfile?.specialties || [
              "Business Scaling",
            ],
          }
        : {
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching community data." });
  }
});
