...
router.get("/status/:artisanId", auth, async (req, res) => {
  try {
    ...
    const existing = await MentorshipService.findOne({
      artisanId,
      ambassadorId,
    });
    if (existing) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(201)
        .json({ message: "Mentorship request sent successfully!", mentorship });
    } catch (error) {
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const updatedMentorship = await MentorshipService.acceptMentorship(
        mentorshipId
      );
