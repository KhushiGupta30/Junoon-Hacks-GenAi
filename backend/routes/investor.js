...
router.put("/profile", [auth, authorize("investor")], async (req, res) => {
  try {
    ...
    const { name, contact, bio, interestedCrafts, typicalInvestmentRange, investmentGoals, profilePicture } = req.body;
...
router.get("/dashboard-data", [auth, authorize("investor")], async (req, res) => {
  try {
    ...
    const uniqueArtisanIds = [...new Set(investments.map((inv) => inv.artisan))];
    ...
    if (recentInvestments.length > 0) {
        const artisanDetails = await UserService.findMany({ id: { in: recentInvestments.map(i => i.artisan) }});
        topArtisans = artisanDetails.map(artisan => ({
            id: artisan.id,
            name: artisan.name,
            profilePic: { url: artisan.profile?.avatar || `https://placehold.co/40x40/cccccc/ffffff?text=${artisan.name.charAt(0)}` },
    ...
    const portfolioChartData = {
        ...
        data: [10000, 10250, 10150, 10300, 10450, 10400, 10550].map(v => v + (stats.totalInvested || 0) - 10000), // Adjust mock data based on real total
    };
