...
// --- MiniLineChart Component (Keep as is) ---
const MiniLineChart = ({ title, data, labels, icon, borderColor, bgColor }) => {
  // ... (Chart component code remains the same) ...
    const chartData = {
    labels: labels,
    datasets: [ { label: title, data: data, borderColor: borderColor || '#4285F4', backgroundColor: bgColor || 'rgba(66, 133, 244, 0.1)', tension: 0.3, fill: true, pointRadius: 0, pointHoverRadius: 4, }, ],
  };
  const options = {
...
const ArtisanDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        // --- REVERTED TO OLD, WORKING FETCH LOGIC ---
        // Fetch stats data
  ...
  return (
    ...
    <div className="px-6 md:px-8 py-8 md:py-10">
      <AnimatedSection className="mb-10 pt-8 md:mb-12"> {/* Note: pt-8 here, is this intentional? */}
            ...
            <div className="space-y-3">
              {topProducts.length > 0 ? (
                topProducts.map((product) => (
                  <Link
                    to={product.link || `/artisan/products/edit/${product.id}`}
                    key={product.id}
