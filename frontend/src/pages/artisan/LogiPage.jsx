    ...
    <div className="bg-white p-5 rounded-lg shadow-sm border animate-pulse">
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 space-y-3">
            <div className="flex justify-between items-center bg-gray-100 p-3 rounded">
                ...
                <div className="space-y-1.5 text-right"><SkeletonBase className="h-5 w-16" /><SkeletonBase className="h-3 w-20" /></div>
            </div>
            <div className="flex justify-between items-center bg-gray-100 p-3 rounded">
                <div className="space-y-1.5"><SkeletonBase className="h-4 w-20" /><SkeletonBase className="h-3 w-28" /></div>
                <div className="space-y-1.5 text-right"><SkeletonBase className="h-5 w-14" /><SkeletonBase className="h-3 w-16" /></div>
            </div>
        </div>
    </div>
);
            ...
            <div className="flex-grow lg:w-2/3">
                <AnimatedSection className="mb-8 pt-8 text-center">
                    ...
                    <p className="mt-3 text-gray-700 text-sm">
                        Get the best shipping recommendations for each of your pending orders.
                    </p>
                </AnimatedSection>
                    ...
                    <div className="space-y-6">
                        ...
                        {ordersAwaitingShipment.length > 0 ? (
                                    ...
                                    <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-2">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600">
                                                To: {order.shippingAddress?.name}, <span className="font-medium">{order.logistics?.destinationCity}</span>
                                            </p>
                                            <p className="text-xs text-gray-500 mt-1">
                                                Distance: {order.logistics?.distanceKm} km &bull; Weight: {order.logistics?.packageWeightKg?.toFixed(1)} kg
                                            </p>
                                        </div>
                                        <a href="#" className="text-xs font-medium text-google-blue hover:underline whitespace-nowrap mt-2 sm:mt-0">View Order Details</a>
                                    </div>

                                    <div className="mt-4 pt-4 border-t border-gray-100">
                                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Shipping Recommendations:</h4>
                                        <div className="space-y-3">
                                            {/* Defensive check with optional chaining '?.' to prevent crash if recommendations is missing */}
                                            {order.logistics?.recommendations?.map(rec => (
                                                <div key={rec.partnerName} className="flex justify-between items-center bg-gray-50 p-3 rounded-md border border-gray-200">
                                                    <div>
                                                        <p className="font-bold text-sm text-gray-800">{rec.partnerName}</p>
                            ...
                            <div className="text-center py-16 px-6 bg-white rounded-lg border-2 border-dashed">
                                ...
                                <p className="text-gray-500 mt-2 text-sm">You have no orders currently awaiting shipment.</p>
                            </div>
                        )}
                    </div>
                </AnimatedSection>
                        ...
                        <div className="space-y-4">
                            {packagingTips.map((tip) => (
                                <div key={tip.title} className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <h4 className="font-semibold text-xs text-gray-800">{tip.title}</h4>
