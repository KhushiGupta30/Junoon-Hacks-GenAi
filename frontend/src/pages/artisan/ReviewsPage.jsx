...
import {
    ...
    ReplyIcon,
    XIcon,
    CollectionIcon, // Make sure this is in Icons.jsx
    ChatAltIcon, // Make sure this is in Icons.jsx
    FilterIcon, // Make sure this is in Icons.jsx
    CalendarIcon, // Make sure this is in Icons.jsx
    TagIcon, // For empty state
    ArchiveIcon // For empty state
...
const SkeletonStatCard = () => <SkeletonBase className="h-28 rounded-xl" />; // Use rounded-xl
    ...
    <div className="bg-white p-5 rounded-lg border border-gray-200 animate-pulse space-y-4">
        <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
                ...
                <div className="space-y-1.5"><SkeletonBase className="h-4 w-24" /><SkeletonBase className="h-3 w-32" /></div>
            </div>
            <SkeletonBase className="h-4 w-16" />
        </div>
        <SkeletonBase className="h-3 w-full" /><SkeletonBase className="h-3 w-2/3" />
        <div className="flex justify-end"><SkeletonBase className="h-8 w-20 rounded-md" /></div>
    </div>
);
// --- End Skeletons ---
...
const ReviewCard = ({ review, onReplySubmit }) => {
    ...
    return (
        <div className="bg-white p-5 rounded-lg border border-gray-200 hover:shadow-md transition-shadow duration-200">
            ...
            <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                    <img src={review.product.image} alt={review.product.name} className="w-10 h-10 rounded-md object-cover flex-shrink-0" />
                    <div className="min-w-0">
                        <p className="font-semibold text-sm text-gray-800 truncate">{review.product.name}</p>
                        <p className="text-xs text-gray-500">by {review.customerName}</p>
                    </div>
                </div>
                 <div className="flex-shrink-0 flex flex-col items-end">
                    <StarRating rating={review.rating} />
                    <p className="text-xs text-gray-400 mt-1">{new Date(review.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
                 </div>
            </div>
            {/* Review Comment */}
                ...
                {review.reply ? (
                    ...
                    <form onSubmit={handleSubmitReply} className="space-y-2">
                        ...
                        <div className="flex justify-end items-center gap-2">
                            <button type="button" onClick={() => setShowReplyForm(false)} className="px-3 py-1 text-xs font-medium text-gray-600 hover:bg-gray-100 rounded-md">Cancel</button>
                            <button type="submit" disabled={isReplying} className="px-3 py-1 text-xs font-bold text-white bg-google-blue rounded-md hover:bg-opacity-90 disabled:opacity-50">
                                {isReplying ? 'Posting...' : 'Post Reply'}
                            </button>
                        </div>
                    </form>
                ) : (
                    <button onClick={() => setShowReplyForm(true)} className="flex items-center gap-1.5 px-3 py-1 text-xs font-medium text-google-blue bg-blue-50/70 hover:bg-blue-100 rounded-md transition-colors">
...
const ReviewsPage = () => {
    ...
    // --- Main filtering and summary logic ---
    const { summary, filteredReviews } = useMemo(() => {
        ...
        if (selectedDateRange !== 'all') {
            ...
            dateFiltered = reviews.filter(r => new Date(r.createdAt) >= cutoffDate);
        }
        let productFiltered = [...dateFiltered];
        if (selectedProduct !== 'all') {
            productFiltered = dateFiltered.filter(r => r.product?._id === selectedProduct);
        }
        const total = productFiltered.length;
        const needsReply = productFiltered.filter(r => !r.reply).length;
        const avgRating = total > 0 ? (productFiltered.reduce((sum, r) => sum + r.rating, 0) / total).toFixed(1) : '0.0';
        let tabFiltered = (activeTab === 'needsReply') ? productFiltered.filter(r => !r.reply) : productFiltered;
        return {
        ...
        return (
             <div className="flex flex-col lg:flex-row gap-10 px-6 md:px-8 py-8 md:py-10 bg-gradient-to-br from-[#F8F9FA] via-[#F1F3F4] to-[#E8F0FE] min-h-screen">
                <div className="flex-grow space-y-8 md:space-y-10">
                    ...
                    {/* SKELETON FILTERS SECOND */}
                    <div className="flex gap-4"><SkeletonBase className="h-10 w-1/2"/><SkeletonBase className="h-10 w-1/2"/></div>
                    <div className="space-y-4"> <SkeletonReviewCard /> <SkeletonReviewCard /> </div>
                </div>
         ...
         return (
            <div className="flex flex-col justify-center items-center min-h-[calc(100vh-10rem)] px-6 text-center bg-gradient-to-br from-[#F8F9FA] via-[#F1F3F4] to-[#E8F0FE]">
                ...
                <p className="text-gray-600 text-sm mb-6">{error}</p>
                 <button
                    onClick={() => { setLoading(true); fetchPageData(); }}
    ...
    return (
        // Page wrapper with background and 2-column layout
        <div className="flex flex-col lg:flex-row gap-10 px-6 md:px-8 py-8 md:py-10 bg-gradient-to-br from-[#F8F9FA] via-[#F1F3F4] to-[#E8F0FE] min-h-screen">
    ...
    <p className="mt-3 text-gray-700 text-sm">
        Manage feedback from your customers.
                ...
                <div className="border-b border-gray-200 sticky top-16 bg-white/80 backdrop-blur-sm z-30 -mx-6 md:-mx-8 px-6 md:px-8">
                    <div className="flex">
                        <TabButton title="All Reviews" isActive={activeTab === 'all'} onClick={() => setActiveTab('all')} count={summary.total} />
                        <TabButton title="Needs Reply" isActive={activeTab === 'needsReply'} onClick={() => setActiveTab('needsReply')} count={summary.needsReply} />
                    </div>
                </div>

                {/* --- Filter Bar (MOVED DOWN) --- */}
                {/* Added top padding (pt-8) and bottom margin (mb-6) */}
                <div className="flex flex-col sm:flex-row gap-3 pt-8 mb-6"> 
                    {/* Product Filter */}
                    <div className="flex-1">
                         <label htmlFor="productFilter" className="block text-xs font-medium text-gray-600 mb-1.5 flex items-center gap-1.5">
                     ...
                     {/* Date Filter */}
                     <div className="flex-1">
                         <label htmlFor="dateFilter" className="block text-xs font-medium text-gray-600 mb-1.5 flex items-center gap-1.5">
                    ...
                    {filteredReviews.length > 0 ? (
                        ...
                        <AnimatedSection>
                            <div className="text-center py-16 px-6 bg-white rounded-lg border border-gray-200">
                                ...
                                <h2 className="text-xl font-medium text-gray-700">No reviews match your filters</h2>
            ...
            <aside className="lg:w-80 flex-shrink-0 space-y-6 lg:sticky lg:top-24 self-start mt-4 lg:mt-0">
                {/* ... (Sidebar cards remain the same) ... */}
                 <AnimatedSection>
                     <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
                        <div className="flex items-center gap-3 mb-3">
                            <StarIcon className="w-5 h-5 text-yellow-400" />
                            <h3 className="text-base font-medium text-gray-800">Average Rating</h3>
                        </div>
                        <p className="text-4xl font-bold text-gray-800">{summary.avgRating}</p>
                        <p className="text-xs text-gray-500 mt-1">Based on {summary.total} selected review(s).</p>
                     </div>
                </AnimatedSection>
                 <AnimatedSection>
                     <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <h3 className="text-base font-medium text-gray-800">Total Selected Reviews</h3>
                        </div>
                        <p className="text-4xl font-bold text-gray-800">{summary.total}</p>
                     </div>
                </AnimatedSection>
                <AnimatedSection>
                     <div className={`p-5 rounded-xl border ${summary.needsReply > 0 ? 'bg-yellow-50/60 border-yellow-200/80' : 'bg-white border-gray-200'}`}>
                        <div className="flex items-center gap-3 mb-3">
                            <ChatAltIcon className={`w-5 h-5 ${summary.needsReply > 0 ? 'text-yellow-600' : 'text-gray-500'}`} />
                            <h3 className={`text-base font-medium ${summary.needsReply > 0 ? 'text-yellow-900' : 'text-gray-800'}`}>Needs Reply</h3>
                        </div>
                        <p className={`text-4xl font-bold ${summary.needsReply > 0 ? 'text-yellow-700' : 'text-gray-800'}`}>{summary.needsReply}</p>
                     </div>
                </AnimatedSection>
            </aside>
