...
const AmbassadorDetailModal = ({ ambassador, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className={`relative bg-white w-full max-w-md rounded-xl shadow-xl p-6 md:p-8 text-center transform transition-all duration-300 ${
                  ...
                  combinedEvents.map((event) =>
                    event.isManual ? (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-3">
                          <div className="flex-1">
                            ...
                            <p className="text-xs font-medium text-google-red mt-1">
                      ...
                      >
                        <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-3">
                          <div className="flex-1">
                            <h3 className="text-sm font-semibold text-gray-800 hover:text-google-blue">
                              {event.title}
                            </h3>
                            <p className="text-xs font-medium text-google-green mt-1">
                              {event.date}
                            </p>
            ...
            {activeTab === "artisans" && (
              <div className="space-y-4">
                ...
                {communityData.localArtisans.length > 0 ? (
                  ...
                  <p className="text-center text-gray-500 py-10 text-sm">
                    No other artisans found nearby yet.
                  </p>
                )}
              </div>
            )}
                  ...
                  mentorshipRequests.map((request) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <img
                          src={
                            request.ambassador?.profile?.avatar ||
                            "https://placehold.co/100x100/4285F4/FFFFFF?text=A"
                          }
                          alt={request.ambassador?.name || "Ambassador"}
                          className="h-10 w-10 rounded-full"
                        />
                        <div>
                          <p className="font-semibold text-sm text-gray-800">
                            {request.ambassador?.name || "Ambassador"}
                          </p>
                          <p className="text-xs text-gray-500">
          ...
          <AnimatedSection>
            <div className="bg-blue-50/60 p-6 rounded-xl border border-blue-200/80">
              ...
              <div className="flex flex-col items-center text-center">
                <img
                  src={communityData.areaAmbassador.avatar}
                  alt={communityData.areaAmbassador.name}
                  className="h-16 w-16 rounded-full border-4 border-white shadow-md mb-2 mx-auto"
                />
                <h4 className="text-sm font-semibold text-gray-800">
                  {communityData.areaAmbassador.name}
                </h4>
                <p className="text-xs text-gray-500 font-medium mb-3">
                  {communityData.areaAmbassador.title}
                </p>
                <button
              ...
              <div className="divide-y divide-gray-100 max-h-80 overflow-y-auto">
                {discussions.length > 0 ? (
                  discussions.map((topic) => (
                    <Link
                      to={`/artisan/discussions/${topic._id}`}
                      key={topic._id}
