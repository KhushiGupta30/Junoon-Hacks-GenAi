...
const BrowseArtisans = () => {
  ...
  const aiMatchedArtisans = artisans.slice(0, 2);
  ...
  const filteredArtisans = useMemo(() => {
    return artisans.filter((artisan) => {
      const searchMatch =
        ...
        artisan.artisanProfile?.craftSpecialty
          ?.join(" ")
          .toLowerCase()
      ...
      <div className="pt-20 flex-grow lg:w-2/3">
        <AnimatedSection className="mb-8 text-center">
          ...
          <p className="mt-3 text-gray-700 text-sm">
            Browse profiles of talented artisans and find opportunities to
            invest.
          </p>
        </AnimatedSection>
        ...
        <AnimatedSection>
          <div className="bg-blue-50/60 p-5 rounded-xl border border-blue-200/80">
            ...
            <p className="text-xs text-gray-700 mb-4 leading-relaxed">
              Based on your profile, these artisans might be a great investment
              fit.
            </p>
            <div className="space-y-3">
              {aiMatchedArtisans.length > 0 ? (
                aiMatchedArtisans.map((artisan) => (
                  <Link
                    to={`../seller/${artisan.id}`}
                    key={artisan.id}
                    className="block bg-white p-3 rounded-lg border border-gray-200 hover:shadow-md transition-shadow"
                  >
                ...
                <p className="text-xs text-gray-600 text-center py-4">
                  No specific matches found right now.
                </p>
              )}
            </div>
          </div>
