...
import api from '../../api/axiosConfig'; // Adjust path
...
const TabButton = ({ title, isActive, onClick, count }) => (
    <button
        ...
        className={`flex items-center gap-2 px-3 py-2 text-sm transition-colors relative whitespace-nowrap ${
            isActive ? 'text-google-blue font-medium' : 'text-gray-500 hover:text-gray-700'
        }`}
        aria-current={isActive ? 'page' : undefined}
    >
        {title}
        {count !== undefined && (
            <span className={`text-xs font-medium px-1.5 py-0.5 rounded-md ${isActive ? 'bg-google-blue/10 text-google-blue' : 'bg-gray-100 text-gray-600'}`}>{count}</span>
        )}
...
const InvestmentTypeChart = ({ data }) => {
    ...
    const options = {
        ...
        plugins: {
            legend: { position: 'bottom', labels: { font: { size: 11, family: 'Roboto' }, color: '#5F6368', boxWidth: 15, padding: 15 } },
            title: { display: false },
        },
    };
...
const InvestmentCraftChart = ({ data }) => {
     const chartData = {
        ...
        datasets: [{
            data: data.data,
            backgroundColor: ['#4285F4', '#34A853', '#FBBC05', '#EA4335', '#7FBCFF', '#81C995'],
            borderRadius: 4,
        }],
    };
    const options = {
        ...
        scales: {
            y: { beginAtZero: true, grid: { color: '#E0E0E0', drawBorder: false }, ticks: { font: { size: 10, family: 'Roboto' }, color: '#5F6368' } },
            x: { grid: { display: false }, ticks: { font: { size: 10, family: 'Roboto' }, color: '#5F6368' } }
        }
    };
    ...
    return (
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 overflow-hidden">
                <div className={`w-1.5 h-10 rounded-full ${typeColors[investment.type] || 'border-gray-300'}`}></div>
                <img src={investment.artisan.avatar} alt={investment.artisan.name} className="h-10 w-10 rounded-full flex-shrink-0" />
                <div className="overflow-hidden">
                    <p className="font-semibold text-sm text-gray-800 truncate">{investment.artisan.name}</p>
            ...
            <div className="flex items-center gap-4 flex-shrink-0">
                 <div className="text-right hidden sm:block">
                     <p className="font-medium text-sm text-gray-800">${investment.amount.toLocaleString()}</p>
                     <p className="text-xs text-gray-500">Invested</p>
                 </div>
                 <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${statusColors[investment.status] || 'bg-gray-100'}`}>
                    {investment.status.replace('_', ' ').charAt(0).toUpperCase() + investment.status.replace('_', ' ').slice(1)}
                 </span>
                 <Link to={`/seller/${investment.artisan.id}`} className="p-1.5 text-gray-400 hover:text-google-blue hover:bg-blue-50 rounded-full transition-colors">
...
// --- End Investment List Item ---

const InvestmentPortfolio = () => {
    const [investments, setInvestments] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState('');
    const [activeTab, setActiveTab] = useState('portfolio'); // 'portfolio', 'analytics', 'exited'

    const mockInvestments = [
        { id: '1', artisan: { id: 'a1', name: 'Rohan Verma', avatar: 'https://placehold.co/100x100/34A853/FFFFFF?text=RV' }, amount: 500, type: 'Equity', status: 'active', craft: 'Pottery' },
        { id: '2', artisan: { id: 'a2', name: 'Meera Patel', avatar: 'https://placehold.co/100x100/4285F4/FFFFFF?text=MP' }, amount: 250, type: 'Loan', status: 'paid_back', craft: 'Painting' },
        { id: '3', artisan: { id: 'a3', name: 'Sanjay Kumar', avatar: 'https://placehold.co/100x100/FBBC05/FFFFFF?text=SK' }, amount: 700, type: 'Equity', status: 'active', craft: 'Woodwork' },
        { id: '4', artisan: { id: 'a4', name: 'Aisha Khan', avatar: 'https://placehold.co/100x100/DB4437/FFFFFF?text=AK' }, amount: 300, type: 'Pre-order', status: 'active', craft: 'Textiles' },
        { id: '5', artisan: { id: 'a5', name: 'Vikram Choi', avatar: 'https://placehold.co/100x100/7FBCFF/FFFFFF?text=VC' }, amount: 150, type: 'Loan', status: 'defaulted', craft: 'Metalwork' },
    ];
    ...
    const { summary, chartData, activeInvestments, exitedInvestments } = useMemo(() => {
         const active = investments.filter(inv => inv.status === 'active');
        const exited = investments.filter(inv => inv.status !== 'active');
        const total = investments.reduce((sum, inv) => sum + inv.amount, 0);
        const typeData = active.reduce((acc, inv) => { acc[inv.type] = (acc[inv.type] || 0) + inv.amount; return acc; }, {});
        const craftData = active.reduce((acc, inv) => { acc[inv.craft] = (acc[inv.craft] || 0) + inv.amount; return acc; }, {});
        return {
            summary: { totalInvested: total, activeCount: active.length, avgReturn: '4.8%' },
            chartData: { byType: { labels: Object.keys(typeData), data: Object.values(typeData) }, byCraft: { labels: Object.keys(craftData), data: Object.values(craftData) } },
            ...
            <div className="flex-grow lg:w-2/3"> {/* CORRECTED: This div now wraps all main content */}
                ...
                <div className="border-b border-gray-200 mb-8 sticky top-2 bg-white/80 backdrop-blur-sm z-30 -mx-6 md:-mx-8 px-6 md:px-8">
                    <div className="flex space-x-2 overflow-x-auto scrollbar-hide">
                        ...
                        <TabButton title="Exited" isActive={activeTab === 'exited'} onClick={() => setActiveTab('exited')} count={exitedInvestments.length} />
                    </div>
                </div>

                {/* --- Tab Content (FIXED: Moved AnimatedSection inside each tab) --- */}
                    ...
                    <AnimatedSection key="portfolio">
                        <div className="space-y-4">
                            {activeInvestments.length > 0 ? (
                                ...
                                <p className="text-center text-gray-500 py-10 text-sm">You have no active investments.</p>
                            )}
                        </div>
                    </AnimatedSection>
                )}
                    ...
                    <AnimatedSection key="analytics">
                        <div className="space-y-8">
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                                <h2 className="text-base font-medium text-gray-800 mb-4 flex items-center gap-2"><ChartPieIcon className="w-5 h-5 text-gray-500"/> Investments by Type</h2>
                                <InvestmentTypeChart data={chartData.byType} />
                            </div>
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                                 <h2 className="text-base font-medium text-gray-800 mb-4 flex items-center gap-2"><ChartBarIcon className="w-5 h-5 text-gray-500"/> Investments by Craft</h2>
                    ...
                    <AnimatedSection key="exited">
                        <div className="space-y-4">
                            {exitedInvestments.length > 0 ? (
                                exitedInvestments.map(inv => <InvestmentCard key={inv.id} investment={inv} />)
                            ) : (
                                <p className="text-center text-gray-500 py-10 text-sm">You have no exited or paid back investments.</p>
                            )}
                        </div>
                    </AnimatedSection>
                )}

            </div> {/* <-- CORRECTED: Closing div for main content area */}
                ...
                <AnimatedSection>
                    <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <h3 className="text-base font-medium text-gray-800">Total Invested</h3>
                        </div>
                        <p className="text-4xl font-bold text-gray-800">${summary.totalInvested.toLocaleString()}</p>
                    </div>
                </AnimatedSection>
                <AnimatedSection>
                    <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <h3 className="text-base font-medium text-gray-800">Active Investments</h3>
                        </div>
                        <p className="text-4xl font-bold text-gray-800">{summary.activeCount}</p>
                 ...
                 <AnimatedSection>
                    <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <h3 className="text-base font-medium text-gray-800">Est. Avg. Return</h3>
                        </div>
                        <p className="text-4xl font-bold text-gray-800">{summary.avgReturn}</p>
                ...
                <AnimatedSection>
                    <div className="bg-blue-50/60 p-5 rounded-xl border border-blue-200/80">
                        ...
                        <p className="text-xs text-gray-700 leading-relaxed mb-4">
                            Your portfolio is heavily weighted in Equity. Consider diversifying with Loan-based investments to balance risk.
                        </p>
                        <Link to="/buyer" className="w-full block text-center bg-white text-google-blue border border-blue-200 font-medium py-1.5 rounded-lg hover:bg-blue-100/70 transition-colors text-xs">
