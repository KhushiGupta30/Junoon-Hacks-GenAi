...
const MyArtisans = () => {
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-6">My Mentored Artisans</h1>
            
            {loading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {[...Array(3)].map((_, i) => <ArtisanCardSkeleton key={i} />)}
                </div>
            ) : error ? (
                <p className="text-red-500 bg-red-100 p-4 rounded-lg">{error}</p>
            ) : artisans.length === 0 ? (
                <p className="text-gray-600">You are not currently mentoring any artisans.</p>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {artisans.map(artisan => (
                        <div key={artisan.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                            ...
                            <div className="mt-4 flex space-x-2">
                                <Link to={`/seller/${artisan.id}`} className="flex-1 text-center bg-gray-100 text-gray-800 font-semibold py-2 px-4 rounded-md text-sm hover:bg-gray-200 transition">
                                    View Profile
                                </Link>
                                <a href={`mailto:${artisan.email}`} className="flex-1 text-center bg-blue-500 text-white font-semibold py-2 px-4 rounded-md text-sm hover:bg-blue-600 transition">
                                    Contact
                                </a>
                            </div>
                        </div>
