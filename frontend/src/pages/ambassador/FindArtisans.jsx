...
const ArtisanCard = ({ artisan }) => {
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      try {
        ...
        console.error(
          "Failed to fetch mentorship status for artisan:",
          artisan.id,
          error
        );
  ...
  const renderButton = () => {
    switch (status) {
      case "available":
        return (
          <button
            ...
            className="flex items-center justify-center w-full px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors duration-200"
          >
      ...
      case "pending":
        return (
          <p className="text-sm text-center font-semibold text-yellow-600">
            Request Pending
          </p>
        );
      case "mentored_by_other":
        return (
          <p className="text-sm text-center text-gray-500">Already Mentored</p>
        );
      case "active":
        return (
      ...
      case "sending":
        return (
          <button
            disabled
            className="flex items-center justify-center w-full px-4 py-2 text-sm font-semibold text-white bg-blue-400 rounded-lg cursor-not-allowed"
          >
            Sending...
          </button>
        );
  ...
  return (
    <div className="p-4 bg-white border rounded-lg shadow-sm flex flex-col justify-between">
      ...
      <div className="mt-4 pt-4 border-t border-gray-100">{renderButton()}</div>
    </div>
  );
};
      ...
      try {
        ...
        setError(
          err.response?.data?.message ||
            "Could not fetch artisans. Please try refreshing the page."
        );
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const filteredArtisans = useMemo(() => {
    ...
    return artisans.filter(
      (artisan) =>
        ...
        artisan.artisanProfile?.craftSpecialty?.some((craft) =>
    ...
    <div>
      <div className="bg-white p-6 rounded-lg shadow-sm border mb-8">
        ...
        <p className="text-gray-600 mb-4">
          Discover and connect with talented artisans across the platform. Your
          mentorship can make a real difference.
        </p>
