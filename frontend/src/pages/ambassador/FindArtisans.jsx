...
const ArtisanCard = ({ artisan, currentAmbassadorId }) => {
    ...
    const handleRequest = async () => {
        setStatus('sending'); // A temporary state for better UX
        try {
            await api.post(`/mentorship/request/${artisan.id}`);
            setStatus('pending');
        } catch (error) {
            alert(error.response?.data?.message || "Failed to send request. The artisan may no longer be available.");
            setStatus('available'); // Revert on error
        }
    };
        ...
        <div className="p-4 bg-white border rounded-lg shadow-sm flex flex-col justify-between">
            <div className="flex items-center space-x-4">
                 ...
                 <div className="flex-1">
                     ...
                     <p className="text-sm text-gray-500 capitalize">{artisan.artisanProfile?.craftSpecialty?.join(', ') || 'General Craft'}</p>
                 </div>
            </div>
    ...
    return (
        <div>
            <div className="bg-white p-6 rounded-lg shadow-sm border mb-8">
                ...
                <p className="text-gray-600 mb-4">Discover and connect with talented artisans who are looking for guidance. Your mentorship can make a real difference.</p>
            ...
            {!loading && !error && filteredArtisans.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filteredArtisans.map(artisan => <ArtisanCard key={artisan.id} artisan={artisan} currentAmbassadorId={user.id} />)}
                </div>
            )}
            ...
            {!loading && !error && filteredArtisans.length === 0 && (
                 <div className="text-center py-10 bg-gray-50 rounded-lg border">
                    ...
                    <h3 className="text-xl font-semibold text-gray-700">No Artisans Found</h3>
                    <p className="text-gray-500">
                        {searchTerm ? `No artisans match your search for "${searchTerm}".` : "It looks like all artisans currently have a mentor. Great work!"}
                    </p>
                </div>
            )}
        </div>
